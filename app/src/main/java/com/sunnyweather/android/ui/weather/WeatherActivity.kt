...
class WeatherActivity : AppCompatActivity() {
    ...
    private fun showWeatherInfo(weather: Weather) {
        ...
        for (i in 0 until days) { //until 左闭右开的区间
        ...
        //现在api没有这项数据了，所以要注释掉，否则会出现空指针
       /* val carWashingText: TextView = findViewById(R.id.carWashingText)
        carWashingText.text = lifeIndex.cardWashing[0].desc*/


        //让ScrollView变成可见的状态
        val weatherLayout: ScrollView = findViewById(R.id.weatherLayout)
        weatherLayout.visibility = View.VISIBLE //让ScrollView变成可见的状态
    }
}